...
-------- 06.01.2020 - Roma --------
    1. Исправлена инициализация полей класса в конструкторе(теперь шаблон инициализируется если template != null, если же template пустая строка, то инициализируется дефолтный шаблон)
...
-------- 05.01.2020 - Roma --------
    1. Добавлены типы полям класса
    2. Исправлены параметры передаваемые в конструктор
    3. Добалены в конструктор два дефолтныых параметра
    4. Для коректной передачи пути исправленна переменая __path(замена символа теперь происходит только в $id)

-------- 04.01.2020 - Roma --------
    1. Создание базового класса для компонентов
